...
export default function useRender(width, height, data, count, source, levels, containerRef, callback){
    ...
    useEffect(()=>{
		if(data != null && data != undefined && count > 0){
			...
			sunburst.render(data, count, source, levels, containerRef.current);
		}
    }, // Render 
