...
export default function Vis(){
	...
	function _render(container, svg, data, source){
		if(svg == null  || data == null || data == null)
			return;
		...
		const fileAccessor = source===PixelCorpusSource.certainty
				? x=>x.file
				: x=>x.file_name,
			docOrder = self._docSorting(data.filtered, fileAccessor),
			freeSpace = container.clientWidth - (self._padding*2 + self._legendWidth + self._docNameWidth);
